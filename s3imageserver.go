...
func imageHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(os.Stdout, "Got if-modified-since: %s\n", ifModifiedSince)
	...
	resizeDuration := time.Since(start)
	...
	fmt.Fprintf(os.Stdout, "%s get: %f decode: %f resize: %f encode: %f\n", r.URL.Path, getDuration.Seconds(), decodeDuration.Seconds(), resizeDuration.Seconds(), encodeDuration.Seconds())
}
...
func main() {
	...
	http.HandleFunc("/2/", imageHandler)
